...
/* ============================= UNIQUE PCF TRANSACTION NUMBER (Section 26) ============================= */

/* Every physical-cash transaction receives a unique, sequential, never-reused
   reference like "PCF-2026-000001". The running sequence is stored in state and
   minted through App.mintPcfNo so the same number can never be issued twice. */
const PCF_TXN_PREFIX = "PCF";
function formatPcfNo(seq, year) {
  const y = year || new Date().getFullYear();
  return `${PCF_TXN_PREFIX}-${y}-${String(Number(seq) || 0).padStart(6, "0")}`;
}

/* ============================= PHYSICAL CASH & RECONCILIATION (Sections 24–25) ============================= */

/* Cash over / short between the expected cash and the counted physical cash. */
function cashVariance(expected, actual) {
  const exp = round2(expected), act = round2(actual);
  const difference = round2(act - exp);
  let status;
  if (difference === 0) status = "BALANCED";
  else if (difference > 0) status = "CASH OVER";
  else status = "CASH SHORT";
  return {
    expected: exp, actual: act, difference,
...
/* Total cash returned (unused balances handed back) for a branch. */
function cashReturnedForBranch(branchCode, cashReturns) {
  return (cashReturns || [])
    .filter((r) => r.branchCode === branchCode)
    .reduce((s, r) => s + (Number(r.amount) || 0), 0);
}

/* Latest recorded physical cash count for a branch (by date, then entry order). */
function latestCashCount(branchCode, cashCounts) {
  const list = (cashCounts || []).filter((c) => c.branchCode === branchCode);
...
/* Imprest reconciliation for one fund: the fund should equal physical cash on
   hand plus released-but-unliquidated advances plus liquidated-but-unreplenished
   expenses. Returns every figure the Cash Reconciliation screen displays. */
function reconcileFund(fund, disbursements, liquidations, replenishments, cashCounts, cashReturns) {
  const mon = monitoringForFund(fund, disbursements, liquidations, replenishments);
  const approvedFund = Number(fund.beginningBalance) || 0;
  const returned = cashReturnedForBranch(fund.branchCode, cashReturns);
  /* Cash physically expected on hand under the imprest model. */
  const computedPhysicalCash = round2(approvedFund - mon.disbursed + mon.replenished + returned);
  const count = latestCashCount(fund.branchCode, cashCounts);
  const hasCount = !!count;
  const physicalCash = hasCount ? round2(Number(count.actualCash) || 0) : computedPhysicalCash;
  const unliquidated = round2(mon.outstanding);
  const unreplenishedLiquidated = round2(Math.max(0, mon.liquidated - mon.replenished));
  const expectedFund = round2(physicalCash + unliquidated + unreplenishedLiquidated);
  const variance = round2(expectedFund - approvedFund);
  let status;
  if (variance === 0) status = "BALANCED";
  else if (variance > 0) status = "CASH OVER";
  else status = "CASH SHORT";
  /* An unresolved cash count variance always forces a review. */
  if (hasCount && round2(Number(count.variance) || 0) !== 0) status = "FOR REVIEW";
  return {
    approvedFund, physicalCash, computedPhysicalCash, hasCount, count,
    released: round2(mon.disbursed), liquidated: round2(mon.liquidated),
    replenished: round2(mon.replenished), returned: round2(returned),
    unliquidated, unreplenishedLiquidated, expectedFund, variance, status,
  };
}
