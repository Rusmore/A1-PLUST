...
const seedFunds = () => ([
  ...
  { id: "fund-DIS", branchCode: "ST", label: "Disney", custodian: "Pura Barloso", beginningBalance: 700000, status: "Active", lastUpdated: todayISO(), lastUpdatedBy: "System" },
    ...
    <div>
      ...
      <div className="pcp-content">
        <div className="pcp-card" style={{ marginBottom: 18 }}>
          ...
          <div className="pcp-table-wrap">
            <table className="pcp-table">
              <thead><tr><th>Plant</th><th>Branch</th><th>Company</th><th>Custodian</th><th>Beginning Balance</th><th>Available Balance</th><th>Status</th><th>Last Updated</th><th>Last Updated By</th><th></th></tr></thead>
              <tbody>
                {funds.map((f) => {
...
export default function App({ userEmail, userName, onSignOut, userRole, isAdmin, userPlants }) {
  ...
  const allowedPlants = useMemo(() => resolvePlants(userPlants), [userPlants]);
  const plantOptions = useMemo(
    () => PLANTS.filter((p) => allowedPlants.includes(p.code)).map((p) => ({ code: p.code, label: p.label })),
    [allowedPlants]
  );
  ...
  useEffect(() => {
    (async () => {
      ...
      /* One-time migration: the Disney fund moved from the old "D1" sub-branch
         ...
         newly configured PHP 700,000.00. Matched by the fixed fund id so no
         duplicate Fund Master record is created, and re-run is a no-op once the
         fund is already on "ST". Historical transactions tagged "D1" move with
         it so they keep showing up in the Disney dashboard's history. */
      const migrateDisneyBranch = (fs, reqs, disbs, reps) => {
        const funds2 = fs.map((f) => (f.id === "fund-DIS" && f.branchCode === "D1")
          ? { ...f, branchCode: "ST", beginningBalance: 700000, lastUpdated: todayISO(), lastUpdatedBy: "System (branch migration)" }
          : f);
